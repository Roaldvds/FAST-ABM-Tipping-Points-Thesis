...
from model_file import AdaptationModel
import pandas as pd
import numpy as np
...
if __name__ == '__main__':
    parameter_sweep = {'seed': range(12345, 12345+50),
                       'nr_households': [1000],
                       #'savings_rate': [0.05], 
                       ...
                       # grid_width = 5,
                       # grid_height = 5,
                       'flood_depth_in_m': [[99, 3, 1, 0, 0]], # same length as grid_height, first 99 (water)
                       'flood_time':  [[30]],
                       ...
                       'network_structure_file_path': ['../data/network_structures/network_empirical.graphml'],
                       ...
                       #              'wet-proofing': 0.5},
                       'measures_aging': [{'dry-proofing': 15, # 0 if no aging, int for number of years they age
                                    'wet-proofing': 15}, 
                                          {'dry-proofing': 10, 'wet-proofing': 10},
                                          {'dry-proofing': 5, 'wet-proofing': 5},
                                          {'dry-proofing': 30, 'wet-proofing': 30}
                                          ],
                       }
    ...
    model_results = results_df.drop(columns = ['RunId', 'iteration', 'population_density', 'AgentID', 'network_structure_file_path', 'savings', 'income', 
                                               'flood_damage_no_measures', 'flood_damage_w_measures', 'flood_damage_experienced', 
    ...
    model_results.to_csv('/scratch/thoridwagenbla/model_data_250618_i.csv')
    
    agent_results = results_df.drop(columns = ['RunId', 'iteration','population_density', 'network_structure_file_path', 'edges', 
    ...
    agent_results.to_csv('/scratch/thoridwagenbla/agent_data_250618_i.csv')
